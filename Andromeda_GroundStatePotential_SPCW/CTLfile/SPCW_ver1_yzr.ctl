...
(define-param y 10) ; the number of unitcell in y-direction
...
(print "Current position: "curr-r"\n")
	...
	(reset-meep)
	(init-fields)
	(if (= curr-eps myvacuum) (use-output-directory (string-append "out_vac_yzr_yp" (number->string ybgn) "_kx" (number->string kpt))) (use-output-directory (string-append "out_yzr_yp" (number->string ybgn) "_kx" (number->string kpt))))
	(if (not (= curr-r prev-r)) (begin (set! rstring (string-append rstring "r")) (set! prev-r curr-r)))
;	(if (not (= ybgn 0)) (begin (set! xstring (string-append xstring "x")) (set! ybgn 0)))
;	(get-filename-prefix "kpt")
        (run-until endtime
		(at-beginning output-epsilon)
                (after-time 0 (to-appended (string-append (string #\e #\x) (string (string-ref polstring curr-pol)) (string #\k #\x) (number->string kpt) (string #\r) rstring) (at-every dt (in-point (vector3 0 ybgn curr-r) output-efield-x))))
                (after-time 0 (to-appended (string-append (string #\e #\y) (string (string-ref polstring curr-pol)) (string #\k #\x) (number->string kpt) (string #\r) rstring) (at-every dt (in-point (vector3 0 ybgn curr-r) output-efield-y))))
                (after-time 0 (to-appended (string-append (string #\e #\z) (string (string-ref polstring curr-pol)) (string #\k #\x) (number->string kpt) (string #\r) rstring) (at-every dt (in-point (vector3 0 ybgn curr-r) output-efield-z))))
		  )
	)
)
