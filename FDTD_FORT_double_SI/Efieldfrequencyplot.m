...
function [ffield,Gx,Gy,Gz]=Efieldfrequencyplot(GFresult,zeropos,mu,oneD,twoD)
...
for i1=1:size(ffield)
    if oneD
        Gx(i1)=squeeze((Efield(1,1,i1))).*cc.^2.*eps0./w(i1).^2./mu;
        Gy(i1)=squeeze((Efield(1,2,i1))).*cc.^2.*eps0./w(i1).^2./mu;
        Gz(i1)=squeeze((Efield(1,3,i1))).*cc.^2.*eps0./w(i1).^2./mu;
    elseif twoD
