function [Xplot,Yplot,Ex,Ey,Ez]=Efieldplot_X_Normal(SILresult,nresult,ftrap)
close all
cc=299792458;eps0=8.854e-12;
Efield=SILresult.E;
ffield=SILresult.f;
nresult_x=nresult.index_x;
nresult_y=nresult.index_y;
nresult_z=nresult.index_z;
tmp=abs(ffield-ftrap);
[idx idx]=min(tmp);
closest=ffield(idx);
w=2*pi*closest;

[Xplot,Yplot]=meshgrid(SILresult.y,SILresult.z);
...
caxis(ax1,[0 1])
hold on
[C H2]=contourf(Xplot*10^6,Yplot*10^6,Nx/2,[0.8 1])
hold off
...
save(['Ex_YZ' num2str(lambda,4) 'nm.mat'],'Ex')
saveas(A,['Ix_YZ' num2str(lambda,4) 'nm.png'])
...
save(['Ey_YZ' num2str(lambda,4) 'nm.mat'],'Ey')
saveas(B,['Iy_YZ' num2str(lambda,4) 'nm.png'])
...
save(['Ez_YZ' num2str(lambda,4) 'nm.mat'],'Ez')
saveas(G,['Iz_YZ' num2str(lambda,4) 'nm.png'])
