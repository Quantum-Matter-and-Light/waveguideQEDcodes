function W1PCWunitcell(h,cont,nSiN,x0,y0,z0,a0,wid,thk,ny,r1,r2,r3,s1,s2,offset,Alpha,Ncell);
% wid=110e-9;
% thk=200e-9;
% a0=373e-9;
% ny=8;
...
for i=1:2;
    for j=1:2;
        ...
        lset(h,cont,'z',z0);
        lset(h,cont,'z span',thk);
        % lset(h,cont,'equation 1',[num2str(elpb) '*sqrt(' num2str(1) '-(x/' num2str(elpa) ')^' num2str(2) ')']);
        % lset(h,cont,'equation 2',[num2str(-1) '*' num2str(elpb) '*sqrt(' num2str(1) '-(x/' num2str(elpa) ')^' num2str(2) ')']);
...
N1 = Ncell/2; %% changed to get 50 unit cell // Kyung
ddx=x0-a0/2;
object_counter=1;
